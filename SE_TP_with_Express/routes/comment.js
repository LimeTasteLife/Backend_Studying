...
const Query_Get_Comment_Post =
  'SELECT c.content c.created_at u.name FROM comment c JOIN user u ON u.id = c.user_id WHERE c.post_id = :post_id ORDER BY c.created_at ORDER BY DESC';
...
router.get('/user', async (req, res, next) => {
  try {
    ...
    const findCommentwithUser = await Comment.findAll({
      ...
      attributes: ['content', 'create_at', 'post_id'],
